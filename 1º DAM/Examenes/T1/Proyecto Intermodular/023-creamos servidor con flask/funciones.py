...
def insertaDocumentaciongrafica(Titulo,Imagen,Texto):           # Funcion para insertar un nuevo registro en DG 
    cursor = conexion.cursor(dictionary = True)                 # Creo un cursor y le damos el dictionary para que sea un Json
    peticion = f"""                                             
...
def actualizaDocumentaciongrafica(Identificador,Titulo,Imagen,Texto):       # Funcion para actualizar los datos del DG
    if Titulo != "":
        actualizaDato(Identificador,"Titulo",Titulo)            # Si el Titulo no está vacío, llamamos la funcion actualizadato
    if Imagen != "":
